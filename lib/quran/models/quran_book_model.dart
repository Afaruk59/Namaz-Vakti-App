...
class QuranBook {
  ...
  String getPageImageUrl(int pageNumber) {
    int apiPageNumber = pageNumber + 1; // API'de sayfa numaraları 1'den başlıyor
  ...
  String getLocalizedFormatName(String format, BuildContext context) {
    final l10n = AppLocalizations.of(context);
    switch (format) {
      case 'Mukabele':
        return l10n?.quranFormatMukabele ?? 'Mukabele';
      case 'Hat 1':
        return l10n?.quranFormatHat1 ?? 'Hat 1';
      case 'Hat 2':
        return l10n?.quranFormatHat2 ?? 'Hat 2';
      default:
        return format;
    }
  }

  List<String> getLocalizedAvailableFormats(BuildContext context) {
    return quranFormats.keys.map((format) => getLocalizedFormatName(format, context)).toList();
  }
