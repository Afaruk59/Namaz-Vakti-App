...
class QuranMediaController {
  ...
  QuranMediaController({
    ...
    required AudioPlayerService audioPlayerService, // Keep parameter for compatibility but don't store
  ...
  Future<void> stopService() async {
    ...
    try {
      ...
      await _channel
          .invokeMethod('updatePlaybackState', {'state': STATE_STOPPED});
      await Future.delayed(Duration(milliseconds: 100));
      await _channel.invokeMethod('stopService');
      _isServiceRunning = false;
    } catch (e) {
  ...
  Future<void> updatePlaybackState(int state) async {
    try {
      ...
      await _channel
          .invokeMethod('updatePlaybackState', {'state': state.toInt()});
  ...
  Future<void> updateMetadata({
    ...
    if (_isServiceRunning) {
      ...
      if (pageNumber > 0 && context != null) {
        final localizedPageNumber = AppLocalizations.of(context)?.pageNumber(pageNumber) ?? "Sayfa $pageNumber";
  ...
  Future<void> updatePosition(int positionMs) async {
    if (_isServiceRunning) {
      await _channel
          .invokeMethod('updatePosition', {'position': positionMs.toInt()});
    }
  }

  /// Kuran sayfası için medya kontrollerini güncelleme
  Future<void> updateForQuranPage(
      int pageNumber, String surahName, int ayahNumber, {BuildContext? context}) async {
    try {
    ...
    _channel.setMethodCallHandler((call) async {
      try {
        ...
        if (playingBookCode != null && playingBookCode != 'quran') {
          print('QuranMediaController: Kitap sistemi aktif, Kuran method call\'ı işlenmedi: ${call.method}');
          return false;
        }
