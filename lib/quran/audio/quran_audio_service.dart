...
class QuranAudioService extends ChangeNotifier
  ...
  bool _isUserInitiated =
      true; // Kullanıcı tarafından başlatılan ses çalma işlemi
  ...
  /// Mevcut sure ve ayet bilgilerini çok dilli olarak döndürür
  String getLocalizedCurrentSurahAndAyah(BuildContext context) {
    if (_isBesmelePlaying || _wasBesmelePlaying) {
      return AppLocalizations.of(context)?.besmele ?? "Besmele";
    }

    final surahName = _repository.getSurahName(_currentSurah);
    final localizedSurahName = SurahLocalizationService.getLocalizedSurahName(surahName, context);
    return "$localizedSurahName:$_currentAyah";
  }
  ...
  QuranAudioService()
      : _audioPlayer = QuranAudioPlayer(),
        ...
        _audioPlayerService = AudioPlayerService.forContext('quran') {
    _initListeners();
  }
  ...
  void _onAudioPlayerUpdate() {
    ...
    if (!_isAyahChanging) {
      ...
      if (wasPlaying != _isPlaying) {
        ...
        if (!_isPlaying && _isBesmelePlaying) {
          print(
              'Besmele duraklatıldı - AudioPlayer.isPlaying: ${_audioPlayer.isPlaying}');
        ...
        if (_isPlaying &&
            (_wasBesmelePlaying || getCurrentSurahAndAyah() == "Besmele")) {
          print(
              'Besmele devam ettiriliyor - AudioPlayer.isPlaying: ${_audioPlayer.isPlaying}');
    ...
    _wordTracker.updateWordTrack(_audioPlayer.position,
        isAyahChanging: _isAyahChanging);
  ...
  void _onWordTrackerUpdate() {
    ...
    if (_isLastAyahOfPage &&
        _wordTracker.currentWordIndex >= 0 &&
        _lastAyahWordCount > 0) {
      ...
      if (_wordTracker.currentWordIndex >= _lastAyahWordCount - 1) {
        print(
            'Son ayetin son kelimesi okunuyor, ses dosyası bitince sayfa değişecek');
  ...
  @override
  Future<void> addPageLastAyahInfo(
      int pageNumber, Map<String, dynamic> info) async {
  ...
  Future<void> playAyahWithXml(
      int surahNo, int ayahNo, String audioUrl, String xmlUrl) async {
    try {
      if (_isDisposed) return;
