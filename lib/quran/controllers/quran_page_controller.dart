...
class QuranPageController extends ChangeNotifier {
  ...
  String _selectedFont = 'ShaikhHamdullah';
  ...
  final List<Map<String, String>> _availableFonts = [
    {'name': 'ShaikhHamdullah', 'displayName': 'Font 1'},
    {'name': 'AlQuranAli', 'displayName': 'Font 2'},
    {'name': 'Hasenat', 'displayName': 'Font 3'},
    {'name': 'Arabtype', 'displayName': 'Font 4'},
    {'name': 'Majalla', 'displayName': 'Font 5'},
    {'name': 'Simpo', 'displayName': 'Font 6'},
    {'name': 'Trado', 'displayName': 'Font 7'},
    {'name': 'Artro', 'displayName': 'Font 8'},
    {'name': 'Artrbdo', 'displayName': 'Font 9'},
    {'name': 'Majallab', 'displayName': 'Font 10'},
    {'name': 'Simpbdo', 'displayName': 'Font 11'},
    {'name': 'Tradbdo', 'displayName': 'Font 12'},
  ];
  ...
  QuranPageController({
    ...
    _currentPage = initialPage.clamp(0, 604); // Sayfa numarasını 0-604 arasında sınırla
    _quranBook = QuranBook(selectedFormat: initialFormat);
    _pageController = PageController(initialPage: _currentPage); // PageController da 0'dan başlar
    print('QuranPageController: initialPage = $initialPage, _currentPage = $_currentPage, PageController initialPage = $_currentPage');
  ...
  Future<void> _loadSavedPreferences() async {
    ...
    print('_loadSavedPreferences: savedPage = $savedPage, savedFormat = $savedFormat, _currentPage = $_currentPage');
    
    // SharedPreferences'dan font boyutunu ve font tipini yükle
    try {
      final prefs = await SharedPreferences.getInstance();
      
      // Kaydedilmiş yazı boyutunu yükle
      final savedFontSize = prefs.getDouble('quran_font_size');
      if (savedFontSize != null) {
        _fontSize = savedFontSize;
        print('Loaded font size: $_fontSize');
      }
      
      // Kaydedilmiş font tipini yükle
      final savedFont = prefs.getString('quran_font');
      if (savedFont != null && _availableFonts.any((f) => f['name'] == savedFont)) {
        _selectedFont = savedFont;
        print('Loaded font: $_selectedFont');
      }
    } catch (e) {
      print('Font preferences load error: $e');
    }

    // Only use the saved page if initialPage is 0 (default value)
    // If initialPage was explicitly set, respect that value
    if (_currentPage == 0) {
      print('Using saved page: $savedPage');
      _currentPage = savedPage;
      _pageController.jumpToPage(_currentPage); // PageController da 0'dan başlar
    } else {
      print('Using explicit initial page: $_currentPage');
    ...
    print('Current font: $_selectedFont');

    // Kaydedilmiş yazı boyutunu yükle
    print('Current font size: $_fontSize');
  ...
  Future<void> setFont(String font) async {
    ...
    // Font tipini SharedPreferences'a kaydet
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('quran_font', font);
      print('Font saved: $font');
    } catch (e) {
      print('Font save error: $e');
    }
  }
  ...
  Future<void> setFontSize(double size) async {
    ...
    // Yazı boyutunu SharedPreferences'a kaydet
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setDouble('quran_font_size', size);
      print('Font size saved: $size');
    } catch (e) {
      print('Font size save error: $e');
    }
  }
  ...
  void changeFormat(String newFormat) {
    ...
    _pageController = PageController(initialPage: currentPageBeforeFormatChange); // PageController da 0'dan başlar
    ...
    print('Format değişimi tamamlandı: Sayfa: $_currentPage, Format: $newFormat');
  ...
  void changePage(int pageNumber) {
    ...
    // PageView'ı güncelle - PageController da 0'dan başlar
  ...
  void onPageChanged(int index) {
    // PageController'dan gelen index 0-based, bizim sayfa numaramız da 0-based
    final pageNumber = index;
    
    // Sayfa numarası kontrolü
    if (pageNumber < 0 || pageNumber > 604) {
      print('Geçersiz sayfa numarası: $pageNumber (index: $index)');
      return;
    }
  ...
  void _cleanPageCache(int currentPage) {
    final keysToRemove = _pageDataCache.keys
        .where((page) => page != currentPage && page != currentPage - 1 && page != currentPage + 1)
        .toList();
