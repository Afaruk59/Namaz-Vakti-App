...
class QuranAudioController with ChangeNotifier {
  ...
  bool _isDetached =
      false; // Add this flag to track if controller is detached but not disposed
  ...
  void updateMediaController() {
    ...
    if (showAudioProgress) {
      ...
      if (audioService.isBesmelePlayingOrWas) {
        ...
        surahName = audioService.currentSurah > 0
            ? audioService.getCurrentSurahName()
            : "Kuran-ı Kerim";
  ...
  void _onAudioServiceUpdate() {
    ...
    if (newPage != pageController.currentPage &&
      ...
      if (audioService.isBesmelePlaying) {
        ...
        print(
            'Otomatik sayfa değişimi yapılıyor: ${pageController.currentPage} -> $newPage');
    ...
    if (audioService.isPlaying &&
        !pageDataCache.containsKey(pageController.currentPage)) {
      pageDataCache[pageController.currentPage] =
          pageController.loadCurrentPageData();
    }
  ...
  void stopAudio() async {
    ...
    if (showAudioProgress || audioService.isBesmelePlaying) {
      ...
      await mediaController
          .updatePlaybackState(QuranMediaController.STATE_STOPPED);
      ...
      try {
        ...
        print('QuranAudioController: playing_book_code temizlendi, kitap sistemi handler\'ları aktif olabilir');
      } catch (e) {
  ...
  Future<void> playAudio() async {
    ...
    try {
      ...
      final shouldPlayBesmele =
          currentAyahNo == 1 && currentSurahNo != 9 && currentSurahNo != 1;

      // Ses dosyası ve XML için aynı ayeti kullan
      final audioUrl =
          audioRepository.getAudioUrl(currentSurahNo, currentAyahNo);
  ...
  void _preloadAdjacentPageData(int currentPage) {
    try {
      ...
      if (currentPage > 0 && !pageDataCache.containsKey(currentPage - 1)) {
        pageDataCache[currentPage - 1] = pageController.takipliService
      ...
      if (currentPage > 1 && !pageDataCache.containsKey(currentPage - 2)) {
        pageDataCache[currentPage - 2] = pageController.takipliService
            .getPageData(currentPage - 2)
            .catchError((error) {
  ...
  Future<void> loadAndPlayCurrentPage() async {
    try {
      ...
      if (!audioService.isDisposed &&
          (audioService.isPlaying || audioService.isBesmelePlaying)) {
        try {
  ...
  Future<void> cleanup() async {
    try {
      ...
      print(
          'QuranAudioController cleanup sırasında mediaController temizlenemedi: $e');
    }
  }
